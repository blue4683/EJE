...
## 0. 지금 저장소 상태 — 어디까지 착수 가능한가

> 이 절은 문서 세트를 저장소에 넣은 시점의 실제 상태입니다. 선행 항목이 갖춰지면 갱신하세요.

| 선행 항목 | 상태 | 이것이 없으면 막히는 작업 |
| --- | --- | --- |
| `docs/api/API명세서.md` v3.0.0 | **선행 필요** | 필드·상태코드·에러코드의 최종 근거. 문서와 어긋나면 **명세가 이깁니다** |
| `docker-compose.yml` | **미구축** | 각 작업 §1 착수 절차의 `docker compose …` 명령 |
| `db/schema.sql` · `db/seed-dev.sql` | **미구축** | A5·A6·A7·A8·B4·B5·B6·B7의 화면 검증 데이터 |
| 백엔드 공개 API 21개 | **미구현** (컨트롤러 0개) | 네트워크를 타는 모든 작업 |

**그래서 오늘 당장 착수 가능한 것은 `Step 0` · `B1` · `B2` 세 개입니다.**
이 셋은 백엔드·DB에 전혀 의존하지 않습니다(B1·B2는 순수 브라우저 API, Step 0은 빌드 설정과 골격).

A 계열 조회 화면은 **백엔드 Step 0(스키마 + `seed-dev.sql`)이 올라온 직후** 전부 검증됩니다.
B3 이후는 해당 엔드포인트가 구현된 뒤입니다.

**설계·분업 자체는 선행 항목과 무관하게 지금 확정되어 있습니다.** 아래 §2·§5·§7이 그 근거입니다.
...
- A의 조회 화면 8개는 백엔드 Step 0(`db/seed-dev.sql` + `schema.sql`)만 올라오면 전부 검증됩니다.

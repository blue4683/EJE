...
const authCanSubmit = computed(() => (
  !authSubmitting.value && !validateEmail(authForm.email) && !validatePassword(authForm.password)
))
...
async function loginAndSubmit() {
  ...
  authError.value = null
  try {
    const data = await logIn({ email: normalizeEmail(authForm.email), password: authForm.password })
  ...
  <section class="record-page">
    ...
    <div v-if="authOpen" class="auth-backdrop" role="presentation" @click.self="closeAuth">
      <section class="auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-title">
        ...
        <h2 id="auth-title">로그인 후 녹음 분석을 계속합니다</h2>
        ...
        <form @submit.prevent="loginAndSubmit">
          ...
          <button class="modal-submit" type="submit" :disabled="!authCanSubmit">{{ authSubmitting ? '로그인 중…' : '로그인 후 분석하기' }}</button>
        </form>
        <router-link class="modal-signup" :to="{ name: 'signup' }">회원가입</router-link>
      </section>
    </div>
...
<style scoped>
.record-page { display: grid; width: min(100%, 720px); gap: var(--space-4); margin: 0 auto; }
.record-card { display: grid; justify-items: center; gap: var(--space-5); padding: 48px var(--space-6) 36px;
